...
export class ProductsService extends ObservableCacheService<Product>{
  TOP_PRODUCTS = `${environment.TOP_PRODUCTS_URL  }`;
  private PRODUCTS_URL = `${environment.API_URL}${environment.PRODUCTS_URL}`;
  ...
  topStoreProducts(storeId: number): Observable<StoreProduct[]> {
    return this.http.get<StoreProduct[]>(`${environment.API_URL}${environment.STORES_URL}/${storeId}/${this.TOP_PRODUCTS}`);
  }
